...
<div class="p-4 md:p-6 lg:p-8 space-y-8">
            ...
            <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="text-lg font-bold text-gray-900">{{ $order->created_at ? \Carbon\Carbon::parse($order->created_at)->format('M d, Y') : 'N/A' }}</div>
                    </div>
                </div>
            </div>
        </div>

    <div class="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-gray-100 mb-8 items-stretch">

    <!-- Column 1: Address Details -->
    <div class="flex flex-col justify-center space-y-2">
        <label class="text-[10px] uppercase tracking-widest font-black text-gray-400 px-1">
            Delivery Destination
        </label>

        <div class="text-lg md:text-xl font-black text-gray-900 leading-tight break-words">
            {{ $order->location ?? 'N/A' }}
        </div>

        
            <p class="text-xs text-gray-500 font-medium bg-gray-50 p-2 rounded-lg border border-gray-100 inline-block mt-2 mb-8">
                <span class="text-primary font-bold">Note:</span> {{ $order->notes ?  $order->notes : 'N/A' }}
            </p>
    ...
    <h4 class="text-lg font-bold text-gray-900 flex items-center">
        <i data-lucide="phone" class="w-5 h-5 me-2 text-primary"></i>
        Call Recipient
    </h4>
    <!-- Column 3: Call -->
    <div class="flex flex-col justify-center space-y-3 pt-4">

        <a href="tel:{{ $order->recipient->phone }}" 
           class="flex items-center space-x-4 group bg-emerald-50/50 
                  hover:bg-emerald-50 p-3 md:p-4 rounded-[2rem] 
                  transition-all duration-300 w-full md:w-auto">

            <div class="w-12 h-12 md:w-14 md:h-14 bg-emerald-500 text-white 
                        rounded-2xl flex items-center justify-center 
                        shadow-xl shadow-emerald-500/20 
                        group-hover:scale-110 transition-transform duration-300">

                <i data-lucide="phone" class="w-5 h-5 md:w-6 md:h-6 animate-pulse"></i>
            </div>
            ...
            <div>
                ...
                <p class="text-sm md:text-base font-black text-gray-900 
